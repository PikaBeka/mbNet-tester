...
int main()
{
    ...
    convolution<<<blocks, threads>>>((float(*)[INPUT_S][INPUT_S])d_input, (float(*)[IF_N][K_S][K_S])d_weight, (float(*)[OUTPUT_S][OUTPUT_S])d_output);
    ...
    printf("Array Tiling convolution finished. It is correct with sizes (%d, %d, %d)\n", IF_N, INPUT_S, OF_N);

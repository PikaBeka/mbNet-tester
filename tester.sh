...
out_path=unroll_cublass # folder names created, output path for created txt files
...
#for j in ${!out_path[@]}; do
    ...
    if [[ ! -d ${out_path} ]] # Create folder if it does not exist
    then
    ...
    for i in ${!C[@]}; do # loop to place all configuration files into use
        ...
        nvprof --log-file ${out_path}/nvprof_comp_${C[$i]}_${HW[$i]}_${K[$i]}.txt ./mbnet # stroe nvprof into the txt file
    done

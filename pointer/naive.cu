...
__global__ void convolution(float *input, float *weight, float *output)
{
    ...
    for (int k = 0; k < IF_N; k++)
    {
        for (int i = 0; i < K_S; i++)
        {
            for (int j = 0; j < K_S; j++)
            {
                tempC += weight[out_ch * IF_N * K_S * K_S + k * K_S * K_S + i * K_S + j] * input[k * INPUT_S * INPUT_S + (output_x + i) * INPUT_S + output_y + j];
            }
        }
    }

    output[out_ch * OUTPUT_S * OUTPUT_S + output_x * OUTPUT_S + output_y] = tempC;
}
...
int main()
{
    ...
    convolution<<<(total + threads - 1) / threads, threads>>>(d_input, d_weight, d_output);
    ...
    printf("Pointer Naive convolution finished. It is correct with sizes (%d, %d, %d)\n", IF_N, INPUT_S, OF_N);
